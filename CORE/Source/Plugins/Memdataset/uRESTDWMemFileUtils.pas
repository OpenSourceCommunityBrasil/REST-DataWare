...
uses
  ...
  {$IFDEF HAS_UNITSCOPE}
  {$IFDEF MSWINDOWS}
  Winapi.Windows, uRESTDWMemWin32,
  {$ENDIF MSWINDOWS}
  System.Classes, System.SysUtils,
  {$ELSE ~HAS_UNITSCOPE}
  {$IFDEF MSWINDOWS}
  Windows, uRESTDWMemWin32,
  {$ENDIF MSWINDOWS}
  Classes, SysUtils,
  {$ENDIF ~HAS_UNITSCOPE}
  uRESTDWMemBase, uRESTDWMemSysUtils;
// Path Manipulation
//
...
uses
  ...
  {$ENDIF MSWINDOWS}
  {$ENDIF ~HAS_UNITSCOPE}
  uRESTDWMemDateTime, uRESTDWMemResources,
  uRESTDWMemStringsB;
{ Some general notes:
...
begin
  uRESTDWMemSysUtils.LoadModule(_Kernel32Handle, kernel32);
...
begin
  Result := False; //IsPrivilegeEnabled(SE_BACKUP_NAME) and IsPrivilegeEnabled(SE_RESTORE_NAME);
end;
