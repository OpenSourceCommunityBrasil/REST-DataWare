...
uses
  ...
  {$ENDIF HAS_UNIT_ANSISTRINGS}
  {$ENDIF ~HAS_UNITSCOPE}
  uRESTDWMemMath, uRESTDWMemResources, uRESTDWMemStringsB,
  uRESTDWMemStringConversions, uRESTDWMemWin32, uRESTDWMemFileUtils;
// memory initialization
procedure ResetMemory(out P; Size: Longint);
begin
  if Size > 0 then
...
//    Result := uRESTDWMemSysUtils.Execute(Format('"%s" %s', [ExeName, CommandLine]), FOutputCallback) = 0
//  else
//    Result := uRESTDWMemSysUtils.Execute(Format('"%s" %s', [ExeName, CommandLine]), FOutput) = 0;
end;
...
function SystemTObjectInstance: TJclAddr;
begin
  Result := 0;//ModuleFromAddr(Pointer(System.TObject));
end;
function IsCompiledWithPackages: Boolean;
begin
