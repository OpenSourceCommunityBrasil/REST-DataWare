...
begin
  {$HINTS OFF}
  if (GetProcessMemoryInfo(GetCurrentProcess, {$IFNDEF FPC}@{$ENDIF}Counters, SizeOf(Counters))) then
...
begin
  ...
  begin
    SetLength(Result, DstLength);
    SetLength(Result, goUtf16ToUtf8(ASource, SrcLength, {$IFNDEF FPC}Result{$ELSE}Pointer(Result){$ENDIF}));
  end;
end;
...
begin
  Result := goUtf8ToUtf16({$IFNDEF FPC}ASource{$ELSE}Pointer(ASource){$ENDIF}, Length(ASource));
end;
...
procedure TRESTDWByteBuffer.Append(const AByte: Byte);
begin
  AppendBuffer(AByte, 1);
end;

procedure TRESTDWByteBuffer.Append(const ABytes: TBytes);
begin
  if Assigned(ABytes) then
    AppendBuffer(ABytes[0], Length(ABytes));
end;

procedure TRESTDWByteBuffer.Append(const ABytes: array of Byte);
begin
  if (Length(ABytes) > 0) then
    AppendBuffer(ABytes[0], Length(ABytes));
end;

procedure TRESTDWByteBuffer.Clear;
begin
  FSize := 0;
end;

constructor TRESTDWByteBuffer.Create(const ACapacity, ADelta: Integer);
var
  D: Integer;
begin
  inherited Create;
...
procedure TRESTDWByteBuffer.AppendBuffer(const ABuffer; const ASize: Integer);
var
  GrowSize: Integer;
begin
...
function TRESTDWByteBuffer.ToBytes: TBytes;
begin
  TrimExcess;
  Result := FBuffer;
end;

procedure TRESTDWByteBuffer.TrimExcess;
begin
  FCapacity := FSize;
  SetLength(FBuffer, FSize);
end;

procedure TRESTDWByteBuffer.Append(const ABytes: TBytes; const AIndex: Integer);
begin
  Append(ABytes, AIndex, Length(ABytes) - AIndex);
end;

procedure TRESTDWByteBuffer.Append(const ABytes: TBytes; const AIndex,
  ASize: Integer);
var
  bSize: Integer;
begin
  ...
  begin
    bSize := Min(ASize, Length(ABytes) - AIndex);
    if (bSize > 0) then
      AppendBuffer(ABytes[AIndex], bSize);
  end;
end;
...
procedure TRESTDWByteBuffer.Append(const ABytes: array of Byte; const AIndex,
  ASize: Integer);
var
  bSize: Integer;
begin
