  ...
  Begin
   If Assigned(TList(Self).Items[Index]) Then
    Begin
     ...
     Begin
       {$IF Defined(FPC) OR not Defined(DELPHI10_4UP)}
  ...
  Else
   Begin
    ...
    If Changes.Count > 0 Then
     BufferStream.InputBytes(VarToBytes(EncodeStrings(Changes.Text{$IFDEF FPC}, TDatabaseCharSet.csUndefined{$ENDIF}), varString));
    If vPrimaryValues = Nil Then
      ...
      Begin
       If TRESTDWClientSQL(Dataset).RecNo > 0 Then
        Begin
         ...
         TMassiveValue(MassiveLineBuff.vMassiveValues.Items[MassiveLineBuff.vMassiveValues.Count -1]).Value := EncodeStrings(vTagKey{$IFDEF FPC}, csUndefined{$ENDIF});
        End;
      End
     ...
     While Not BufferBody.Eof Do
      Begin
       ...
       Try
        ...
        If aBool Then
         MassiveLine.vChanges.Text := DecodeStrings(BytesToVar(BufferStream.ReadBytes, varString){$IFDEF FPC}, TDatabaseCharSet.csUndefined{$ENDIF});
        aBool := BytesToVar(BufferStream.ReadBytes, varBoolean);
        If aBool Then
         Begin
                  ...
                  Begin
                   If (Not (TRESTDWJSONInterfaceObject(bJsonValueB).Pairs[0].isnull))        And
                    ...
                    Begin
                     ...
                     {$IF not Defined(FPC) AND not Defined(DELPHI2009UP)}
                        ...
                        Begin
                         ...
                         If TRESTDWClientSQL(Dataset).FindField(TRESTDWJSONInterfaceObject(bJsonValueY).Pairs[0].Name).DataType in [{$IFDEF DELPHIXEUP}ftLongWord,{$ENDIF}ftLargeint] Then
                          Begin
                           ...
                           {$ELSE}
                            TRESTDWClientSQL(Dataset).FindField(TRESTDWJSONInterfaceObject(bJsonValueY).Pairs[0].Name).AsInteger := StrToInt64(vValue);
                           {$IFEND}
                          End
                         Else
                        ...
                        Begin
                         ...
                         If TRESTDWTable(Dataset).FindField(TRESTDWJSONInterfaceObject(bJsonValueY)
                          ...
                          Begin
                           ...
                           {$ELSE}
                            TRESTDWTable(Dataset).FindField(TRESTDWJSONInterfaceObject(bJsonValueY).Pairs[0].Name).AsInteger := StrToInt64(vValue);
                           {$IFEND}
                          End
                         Else
