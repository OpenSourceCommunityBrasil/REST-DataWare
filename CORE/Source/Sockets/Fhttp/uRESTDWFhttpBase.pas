...

{
  ...
  Anderson Vieira Fiori             - Admin - Gerencia de Organização dos Projetos
...
Uses
  ...
  fphttpclient, fphttpserver,  sslbase, sslsockets, ssockets
  {
...
Type
 ...
 Private
  ...
  vSSLVerifyMode                   : TSSLVerifyMode;
  vSSLVerifyDepth                  : Integer;
  vSSLMode                         : TSSLMode;
  aSSLVersions                     : TSSLType;
...
Type
 ...
 Private
  ...
  Procedure SetHeaders        (AHeaders           : TStringList;
                               Var SendParams     : TStringList{TIdMultipartFormDataStream});Overload;
  Procedure SetRawHeaders     (AHeaders           : TStringList;
                               Var SendParams     : TStringList{TIdMultipartFormDataStream});
  Procedure pOnWork           (Sender: TObject;
                               const ContentLength,
                               CurrentPos: Int64);
 ...
 Public
  ...
  Function   Post      (AUrl              : String         = '';
                        CustomHeaders     : TStringList    = Nil;
                        Const CustomBody  : TStringList{TIdMultipartFormDataStream} = Nil;
                        Const AResponse   : TStream        = Nil;
                        IgnoreEvents      : Boolean        = False;
...
Type
 TIdHTTPAccess = class(TFPHTTPClient)
End;
...
Begin
 ...
 Try
  ...
  Try
   ...
   If Not Assigned(CustomBody) Then
    Begin
     HttpRequest.FormPost(AUrl, SendParams, atempResponse);
     Result:= HttpRequest.ResponseStatusCode;
     if Assigned(OnHeadersAvailable) then
      OnHeadersAvailable(TStringList(HttpRequest.ResponseHeaders), True);
     atempResponse.Position := 0;
   ...
   Else
    Begin
     If Assigned(CustomBody) Then
      HttpRequest.FormPost(AUrl, CustomBody, AResponse)
     Else if Assigned(SendParams) Then
      HttpRequest.FormPost(AUrl, SendParams, AResponse)
     Else
      HttpRequest.FormPost(AUrl, CustomHeaders, AResponse);
     Result:= HttpRequest.ResponseStatusCode;
...
{$ENDIF}
{$ENDIF}
Function TRESTDWFhttpClientREST.IdSSLIOHandlerSocketOpenSSL1VerifyPeer(Certificate : TX509Certificate;
                                                                    AOk         : Boolean;
                                                                    ADepth,
...
Procedure TRESTDWFhttpClientREST.pOnWorkEnd (ASender    : TObject;
                                          AWorkMode  : TWorkMode);
Begin
 OnWorkEnd(ASender);
End;
       ...
       If SendParams = Nil Then
        Begin
         If (AHeaders.Names[i] <> '') Or (AHeaders.ValueFromIndex[i] <> '') Then
          Begin
           If RequestCharset = esUtf8 Then
            HttpRequest.AddHeader(AHeaders.Names[i], utf8Decode(AHeaders.ValueFromIndex[i]))
           Else
       ...
       Else
        Begin
         If (AHeaders.Names[i] <> '') Or (AHeaders.ValueFromIndex[i] <> '') Then
          Begin
           If RequestCharset = esUtf8 Then
            SendParams.AddPair(AHeaders.Names[i],  utf8Decode(AHeaders.ValueFromIndex[i]))
           Else
            SendParams.AddPair(AHeaders.Names[i],  AHeaders.ValueFromIndex[i]);
          End;
        End;
      End;
    End;
   ...
   If SendParams <> Nil Then
    Begin
     {$IFNDEF FPC}
      ...
      {$IFEND}
     {$ELSE}
      SendParams.AddPair('Access-Control-Allow-Origin',  AccessControlAllowOrigin);
     {$ENDIF}
 ...
 If Assigned(AHeaders) Then
  Begin
   If AHeaders.Count > 0 Then
    Begin
     For i := 0 to AHeaders.Count-1 do
      HttpRequest.RequestHeaders.AddPair(AHeaders.Names[i], AHeaders.ValueFromIndex[i]);
    End;
  End;
 {
...
//   Result := '?' + Result;
 End;
 Procedure SetParamsValues(DWParams : TRESTDWParams; SendParamsData : TStringList{TIdMultipartFormDataStream});
 Var
  I            : Integer;
  vCharsset    : String;
 Begin
      ...
      For I := 0 To DWParams.Count -1 Do
       Begin
        If DWParams.Items[I].ObjectValue in [ovWideMemo, ovBytes, ovVarBytes, ovBlob, ovStream,
         ...
         End
        Else
         SendParamsData.AddPair(DWParams.Items[I].ParamName, DWParams.Items[I].ToJSON);
       End;
     End;
   End;
 End;
   ...
   Case EventType Of
    ...
    sePATCH :
     Begin;
      SendParams := TStringList.Create;
      If WelcomeMessage <> '' Then
       SendParams.AddPair('dwwelcomemessage', EncodeStrings(WelcomeMessage{$IFDEF FPC}, DatabaseCharSet{$ENDIF}));
      If AccessTag <> '' Then
       SendParams.AddPair('dwaccesstag',      EncodeStrings(AccessTag{$IFDEF FPC}, DatabaseCharSet{$ENDIF}));
      If ServerEventName <> '' Then
       Begin
        ...
        Try
         ...
         JSONValue.SetValue(ServerEventName, JSONValue.Encoded);
        Finally
         SendParams.AddPair('dwservereventname', JSONValue.ToJSON);
         FreeAndNil(JSONValue);
        End;
       End;
//      Else
...
//         End;
//       End;
      SendParams.AddPair('datacompression',   BooleanToString(Datacompress));
      SendParams.AddPair('dwassyncexec',      BooleanToString(Assyncexec));
      SendParams.AddPair('dwencodestrings',   BooleanToString(EncodedStrings));
      SendParams.AddPair('binaryrequest',     BooleanToString(BinaryRequest));
      If AuthenticationOptions.AuthorizationOption    <> rdwAONone Then
       Begin
        If Assigned(Params) Then
         Begin
           ...
           rdwAOBearer : Begin
                          If TRESTDWAuthOptionBearerClient(AuthenticationOptions.OptionParams).TokenRequestType <> rdwtHeader Then
                           Begin
                            If TRESTDWAuthOptionBearerClient(AuthenticationOptions.OptionParams).Token <> '' Then
                             Begin
                              SendParams.AddPair(TRESTDWAuthOptionBearerClient(AuthenticationOptions.OptionParams).Key,
           ...
           rdwAOToken  : Begin
                          If TRESTDWAuthOptionTokenClient(AuthenticationOptions.OptionParams).TokenRequestType <> rdwtHeader Then
                           Begin
                            If TRESTDWAuthOptionTokenClient(AuthenticationOptions.OptionParams).Token <> '' Then
                             Begin
                              SendParams.AddPair(TRESTDWAuthOptionTokenClient(AuthenticationOptions.OptionParams).Key,
        ...
        Else
         Begin
          Case AuthenticationOptions.AuthorizationOption Of
           rdwAOBearer : Begin
                          If TRESTDWAuthOptionBearerClient(AuthenticationOptions.OptionParams).TokenRequestType <> rdwtHeader Then
                           SendParams.AddPair(TRESTDWAuthOptionBearerClient(AuthenticationOptions.OptionParams).Key, TRESTDWAuthOptionBearerClient(AuthenticationOptions.OptionParams).Token);
                         End;
           rdwAOToken  : Begin
                          If TRESTDWAuthOptionTokenClient(AuthenticationOptions.OptionParams).TokenRequestType <> rdwtHeader Then
                           SendParams.AddPair(TRESTDWAuthOptionTokenClient(AuthenticationOptions.OptionParams).Key,  TRESTDWAuthOptionTokenClient(AuthenticationOptions.OptionParams).Token);
                         End;
          End;
         End;
       End;
      If aBinaryCompatibleMode Then
       SendParams.AddPair('BinaryCompatibleMode', BooleanToString(aBinaryCompatibleMode));
      If Assigned(vCripto) Then
       SendParams.AddPair('dwusecript',       BooleanToString(vCripto.Use));
      If Params <> Nil Then
