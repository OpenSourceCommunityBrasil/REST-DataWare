...
Uses
 ...
 {$ENDIF}
  ,uRESTDWDataUtils,     OverbyteIcsWinSock,       OverbyteIcsWSocket, OverbyteIcsWndControl,
   OverbyteIcsHttpSrv,   OverbyteIcsHttpAppServer, OverbyteIcsUtils,   OverbyteIcsFormDataDecoder,
   OverbyteIcsMimeUtils, uRESTDWTools;
       ...
       If Pooler = Format('%s.%s', [ServerMethodsClass.ClassName, ServerMethodsClass.Components[i].Name]) Then
        Begin
         ...
         If AContext <> Nil Then
          Begin
           ClientCnx := THttpAppSrvConnection(AContext);
           MyIP := ClientCnx.PeerAddr;
          End;
         Break;
        End;
      End;
...
Procedure TRESTDWIcsServicePooler.GetDocument(Sender,
                                              Client    : TObject;
                                              Var Flags : THttpGetFlag);
Var
 ClientCnx : THttpAppSrvConnection;
 ...
 Begin
  ...
  mb.Position                          := 0;
  ClientCnx.DocStream := mb;
  ClientCnx.AnswerStream(Flags,
                         IntToStr(StatusCode), { Default Status '200 OK'            }
...
//  AResponseInfo.Redirect(Url);
 End;
Begin
 ClientCnx := THttpAppSrvConnection(Client);
 ...
 Try
  If CORS Then
   ...
   End;
  vToken       := ClientCnx.AuthDigestUri;
  vAuthRealm   := ClientCnx.AuthRealm;
  vContentType := ClientCnx.RequestContentType;
  vParams      := TStringList.Create;
  vParams.Text := ClientCnx.params;
  If CommandExec  (TComponent(ClientCnx),
                   RemoveBackslashCommands(ClientCnx.Path),
                   ClientCnx.Method + ' ' + ClientCnx.Path,
                   vContentType,
                   ClientCnx.PeerAddr,
                   ClientCnx.RequestUserAgent,
                   ClientCnx.AuthUserName,
                   ClientCnx.AuthPassword,
                   vToken,
                   ClientCnx.RequestHeader,
                   StrToInt(ClientCnx.PeerPort),
                   Nil,
                   vParams,
                   ClientCnx.params,
                   ClientCnx.DocStream,
                   ...
                   vRedirect) Then
   Begin
    ClientCnx.AuthRealm   := vAuthRealm;
    ...
    If Assigned(ResultStream)    Then
     Begin
      ...
      ClientCnx.AnswerStream(Flags,
                             IntToStr(StatusCode), { Default Status '200 OK'            }
                             vContentType  { Default Content-Type: text/html    },
                             vResponseHeader.Text  {Header});
     End;
   End
