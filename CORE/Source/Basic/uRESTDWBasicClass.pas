...
Type
 TRESTDWClientHttpBase = Class(TRESTDWComponent) //Novo Componente de Acesso a Requisições REST para o Servidores Diversos
 Private
  //Variáveis, Procedures e Funções Privadas
  vRSCharset           : TEncodeSelect;
  vLastErrorCode,
 ...
 Public
  ...
  Function   Get       (AUrl                 : String          = '';
                        ...
                        Const AResponse      : TStream         = Nil;
                        Const AResponseError : TStream         = Nil):Integer;Virtual;Abstract;
  Function   Post      (AUrl                 : String          = '';
                        Const CustomHeaders  : TStringList     = Nil;
...
Constructor TRESTDWClientHttpBase.Create(AOwner: TComponent);
Begin
 Inherited;
...
Destructor TRESTDWClientHttpBase.Destroy;
Begin
...
Function TRESTDWClientHttpBase.GetAllowCookies : Boolean;
Begin
 Result := vAllowCookies;
End;

Function TRESTDWClientHttpBase.GetHandleRedirects : Boolean;
Begin
 Result := vHandleRedirects;
End;

Procedure TRESTDWClientHttpBase.SetOnWork(Value : TOnWork);
Begin
  vOnWork := Value;
End;

Procedure TRESTDWClientHttpBase.SetOnWorkBegin(Value: TOnWork);
Begin
  vOnWorkBegin := Value;
End;

procedure TRESTDWClientHttpBase.SetOnWorkEnd(Value: TOnWorkEnd);
begin
  vOnWorkEnd := Value;
end;

procedure TRESTDWClientHttpBase.SetOnStatus(Value: TOnStatus);
begin
  vOnStatus := Value;
end;

Procedure TRESTDWClientHttpBase.SetAllowCookies(Value : Boolean);
Begin
 vAllowCookies := Value;
End;

Procedure TRESTDWClientHttpBase.SetAuthOptionParams(Value : TRESTDWClientAuthOptionParams);
Begin
 vAuthOptionParams.Assign(Value);
End;

Procedure TRESTDWClientHttpBase.SetDefaultCustomHeader(Value: TStrings);
Begin
 vDefaultCustomHeader.Assign(value);
End;

Procedure TRESTDWClientHttpBase.SetUseSSL(Value     : Boolean);
Begin
 vUseSSL := Value;
End;

Function TRESTDWClientHttpBase.GetUseSSL : Boolean;
Begin
 Result := vUseSSL;
End;

Procedure TRESTDWClientHttpBase.SetHandleRedirects(Value : Boolean);
Begin
